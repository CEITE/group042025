...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initialize health monitoring charts only if pets exist
        <?php if (!empty($pets)): ?>
    ...
    function initializeHealthCharts() {
        // FIXED: Check if chart elements exist before initializing
        const vaccinationChartEl = document.getElementById('vaccinationChart');
        const healthScoreChartEl = document.getElementById('healthScoreChart');
        const visitChartEl = document.getElementById('visitChart');
        const weightChartEl = document.getElementById('weightChart');
        
        if (!vaccinationChartEl || !healthScoreChartEl || !visitChartEl || !weightChartEl) {
            console.warn('Chart elements not found. Skipping chart initialization.');
            return;
        }

        // Vaccination Status Chart
        const vaccinationCtx = vaccinationChartEl.getContext('2d');
        if (vaccinationCtx) {
        ...
        // Health Scores Chart
        const healthScoreCtx = healthScoreChartEl.getContext('2d');
        if (healthScoreCtx) {
        ...
        const visitCtx = visitChartEl.getContext('2d');
        if (visitCtx) {
            new Chart(visitCtx, {
                type: 'line',
                data: {
    ...
    function loadDiseaseClasses() {
        // FIXED: Add CORS mode to fetch request
        ...
        .catch(error => {
            ...
            document.getElementById('diseaseClassesList').innerHTML = 
                '<small class="text-muted">Cannot connect to AI service. Please check CORS configuration.</small>';
        });
    }
    ...
    async function analyzePetImage() {
        ...
        try {
            // FIXED: Add CORS mode and handle CORS errors
            ...
            let errorMessage = error.message || 'Unable to connect to AI service. Please try again.';
            
            // Provide more specific error messages
            if (errorMessage.includes('CORS')) {
                errorMessage = 'CORS policy blocked the request. The AI service needs to allow requests from this domain.';
            } else if (errorMessage.includes('Failed to fetch')) {
                errorMessage = 'Cannot connect to AI service. The service might be down or there might be a network issue.';
            }
